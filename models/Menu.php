...
class Menu extends \yii\db\ActiveRecord
{
    ...
    public function getTypes() {
        return [
            1=>Yii::t('admin','Приложение'),
            2=>Yii::t('admin','Категория'),
            3=>Yii::t('admin','Материал'),
            4=>Yii::t('admin','JSON параметры'),
            5=>Yii::t('admin','Произвольная ссылка'),
        ];
    }
    ...
    public function getItems() {
        if (in_array($this->type, [3]) && $this->category_id !== 0) {
            $items = Item::find()->joinWith('categories')->where(['{{%zoo_categories}}.id'=>$this->category_id])->asArray()->all();
    ...
    public function getCheck() {
        ...
        elseif($this->type == 3 && $this->category_id !== null && Item::findOne($this->item_id) !== null) {
            return true;
        }
